...
public class OrderEmailNotifier : IOrderEmailNotifier
{
    ...
    {
        var ordersLink = BuildOrdersLink();
        ...
        {
            sb.AppendLine(
                $"<tr><td>{item.ProductName}</td><td>{item.Quantity}</td><td>{item.Price:C}</td><td>{item.Quantity * item.Price:C}</td></tr>");
        }
        sb.AppendLine("</table>");
        ...
        if (!string.IsNullOrWhiteSpace(ordersLink))
        {
            sb.AppendLine($"<p>Track your order status in your account: <a href='{ordersLink}'>Open My Orders</a></p>");
        }
    ...
    private string BuildOrderStatusUpdateEmailBody(Order order, string newStatus)
    {
        ...
        if (!string.IsNullOrWhiteSpace(ordersLink))
        {
            sb.AppendLine($"<p>View your orders: <a href='{ordersLink}'>Open My Orders</a></p>");
        }
        sb.AppendLine("<p>Sincerely,<br/>PharmacyApp Team</p>");
        sb.AppendLine("</body></html>");
    ...
    private string BuildOrderCancellationEmailBody(Order order)
    {
        ...
        if (!string.IsNullOrWhiteSpace(ordersLink))
        {
            sb.AppendLine($"<p>Check your orders here: <a href='{ordersLink}'>Open My Orders</a></p>");
        }
        sb.AppendLine("<p>Sincerely,<br/>PharmacyApp Team</p>");
        sb.AppendLine("</body></html>");
    ...
    private string BuildOrderCompositionChangeEmailBody(Order order)
    {
        ...
        sb.AppendLine(
            $"<tr style='background-color: #f8f9fa;'><td colspan='3' style='text-align: right; font-weight: bold;'>Total Amount</td><td style='font-weight: bold;'>{order.TotalAmount:C}</td></tr>");
        sb.AppendLine("</table>");

        sb.AppendLine($"<p style='margin-top: 15px;'><strong>Order date:</strong> {order.OrderDate:dd.MM.yyyy HH:mm}</p>");
        if (!string.IsNullOrWhiteSpace(ordersLink))
        {
            sb.AppendLine($"<p>Review all updates in your account: <a href='{ordersLink}'>Open My Orders</a></p>");
        }

        sb.AppendLine(
            "<p style='margin-top: 20px;'>If you have any questions or did not authorize these changes, please contact our customer service immediately.</p>");
        sb.AppendLine("<p>Sincerely,<br/>PharmacyApp Team</p>");
        sb.AppendLine("</body></html>");
    ...
    private string BuildOrdersLink()
    {
        ...
        return $"{_frontendOptions.BaseUrl.TrimEnd('/')}/orders";
    }
}
